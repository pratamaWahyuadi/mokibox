...
=== Jawaban User ===
# Jawaban Discovery — TikTok Clone Backend MVP (Revisi Final)

## 1. Skala & Beban Sistem

1. **Target registered users MVP:** ±1.000 user. Bukan 10K. Fokus ke early adopters dan testing.
2. **Concurrent active user:** 50–100 user. Pola ramai kemungkinan jam malam (19:00–23:00 WIB).
3. **Video per hari:** 20–50 video. **Per bulan:** ±1.000 video. Rata-rata durasi 30–60 detik.
4. **Volume data baru per hari:** ±2–5 GB (sudah termasuk raw + HLS + thumbnail).
5. **Lonjakan traffic:** Demo day ke stakeholder. Tidak ada event publik besar.

## 2. Timeline & Prioritas

6. **Deadline 2 minggu:** Hard deadline. Bisa mundur maksimal 3 hari.
7. **Fitur yang bisa dikorbankan pertama:** Reply comment. Kedua: private account. Ketiga: delete video.
8. **Testing + polish:** Non-negotiable. Minimal basic integration test.
9. **MVP untuk:** Proof-of-concept + demo internal stakeholder.

## 3. Budget & Constraint Infrastruktur

10. **Budget bulanan:** ±Rp 500.000–700.000 (±$30–45).
11. **Zitadel:** Self-host dalam docker-compose yang sama. Tim siap tanggung operational burden.
12. **Deployment:** Satu VPS tunggal. Semua jalan dalam satu docker-compose.
13. **Compliance/data residency:** Tidak ada. Data boleh di region mana pun.

## 4. Tech Stack — Bahasa & Framework

14. **Go + Echo:** Tim familiar dengan Go untuk performa. Echo dipilih karena ringan dan simpel.
15. **Versi Go:** 1.22+.
16. **Library tambahan:** Boleh. Prioritas: `google/uuid`, `golang-jwt/jwt` (atau sejenis), validator via Echo binder. Minimalkan dependency yang tidak perlu.

## 5. Tech Stack — Database

17. **PostgreSQL:** 16 final. Tidak mau ganti versi.
18. **Extension:** `pgcrypto` untuk `gen_random_uuid()`. Tidak perlu `pg_trgm` dulu.
19. **Single instance cukup.** Belum ada rencana read replica.
20. **Migrasi:** File SQL manual via `psql` + Makefile. Tidak perlu tool migration.

## 6. Tech Stack — Akses Data

21. **Standar akses data:** **sqlc** untuk type-safe code generation dari SQL. Tidak pakai GORM, tidak raw SQL manual.
22. **Prioritas akses data:** **Type safety = kontrol penuh atas query** > kecepatan development.
23. **Connection pooling:** Pakai default `pgxpool`. Tidak perlu tuning eksplisit.
24. **Rollback migration:** Tidak perlu. Cukup migration forward-only.

## 7. Tech Stack — Frontend

25. **Ini murni API backend.** Mobile app belum ada. Konsumer: Postman/curl saat ini.
26. **Mobile app:** Belum ada. Rencana awal pakai Flutter, tapi di luar scope MVP ini.
27. **Web frontend:** Tidak perlu. Cukup Postman/curl.
28. **Halaman admin:** Tidak perlu di MVP.

## 8. Tech Stack — Auth

29. **Login flow:** User dibawa ke hosted UI Zitadel. API hanya terima JWT.
30. **Token:** Access token JWT + refresh token. **Refresh token rotation ditangani penuh oleh Zitadel.** Di sisi Go, tidak perlu implementasi logika refresh token. Cukup:
    - Validasi access token JWT via RSA public key.
    - Jika expired → return 401 → client redirect ke Zitadel untuk dapat access token baru.
31. **Role:** Semua user sama. Tidak ada admin/moderator di MVP.
32. **User dihapus/dinonaktifkan di Zitadel:** **Backend harus mendengarkan webhook Zitadel** dan ikut menghapus/menonaktifkan data user di PostgreSQL. Worth it walau MVP, karena menjaga konsistensi data.
33. **Multi-device login:** Boleh. Tidak perlu fitur logout semua perangkat.
34. **Tambahan:** Zitadel harus dikonfigurasi mendukung **Google Sign-In** sebagai identity provider eksternal.

## 9. Tech Stack — Deployment

35. **Target deploy:** VPS pribadi. Misal: Vultr, DigitalOcean, atau IDCloudHost.
36. **Docker-compose:** Dipakai untuk development dan production langsung.
37. **CI/CD:** Tidak perlu. Cukup `git pull` + `docker-compose up -d`.
38. **Domain & reverse proxy:** Sudah punya domain. Pakai Nginx sebagai reverse proxy. Zitadel issuer URL akan pakai subdomain.

## 10. Integrasi Eksternal

39. **Integrasi wajib:** R2, Zitadel, FFmpeg. Tidak ada FCM/APNs/email.
40. **Moderasi konten:** Di luar scope MVP.
41. **Zitadel sign-in:** Email/password internal **+ Google Sign-In**.

## 11. Data & Privasi

42. **PII:** Username, display_name, avatar_url. Email dari Zitadel tidak perlu disimpan di PostgreSQL.
43. **Delete account:** Fitur hapus akun + semua data user (video, like, comment, follow, notifikasi, file R2) **perlu ada** walaupun sederhana.
44. **Retensi data:** Video dihapus → file R2 ikut dihapus maksimal 1x24 jam.
45. **Audit log:** Tidak perlu.

## 12. Offline / Sinkronisasi

46. **Mode offline:** Tidak perlu. Online-only.
47. **Idempotency key:** Tidak perlu di MVP.
48. **Online-only:** Ya, eksplisit online-only.

## 13. Observability & Maintenance

49. **Maintainer:** Orang yang sama yang develop.
50. **Logging:** Cukup stdout dari docker-compose. Belum perlu structured logging.
51. **Monitoring:** Cukup cek manual. Tidak perlu dashboard/alert.
52. **Insiden production:** Best effort. Tidak ada on-call.

## 14. Non-Functional Requirements

53. **Latency:** Feed p95 < 500ms. Like p95 < 200ms. Upload-intent p95 < 300ms.
54. **SLA transcode:** Video 60 detik harus selesai < 5 menit.
55. **Target uptime:** 95%. Tidak ada SLA ketat.
56. **Skalabilitas:** Cukup benar untuk skala kecil (±1.000 user). Bisa di-refactor nanti.

## 15. Edge Case & Kegagalan

57. **Kegagalan paling ditakutkan:** Transcode stuck dan PostgreSQL kehabisan disk.
58. **Zitadel down:** API tolak semua request. Tidak ada mode degraded.
59. **Transcode gagal:** Retry maksimal 3x. Setelah itu status FAILED permanen. User tidak perlu diberi tahu real-time, cukup lihat status.
60. **Presigned URL kadaluarsa:** User bisa minta URL baru. Validasi `r2_key` di confirm: cek bahwa key valid dan user yang konfirmasi adalah pemilik yang sama dengan yang minta upload-intent.
61. **Hapus video saat transcode:** Cek status video sebelum update DB. Kalau video sudah dihapus user, hasil transcode tidak di-publish.
62. **Views count:** Satu user menonton video sama berkali-kali dihitung berkali-kali. Tidak perlu deduplikasi.
63. **Hapus komentar induk:** Semua balasan ikut terhapus sesuai `ON DELETE CASCADE`.